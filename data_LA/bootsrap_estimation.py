...
parser.add_argument("--num_user", type=int, default=572, help="num user to select from in each replication")
...
if __name__ == '__main__':
    ...
    # load vcc2018_mos
    df = pd.read_csv('./LA_mos.csv', encoding='utf-8') 

    # df.iloc[df.loc[df['audio_sample'].str.contains('NAT')].index, df.columns.get_loc('MOS')] = 5

    # remove NAT audio
    df = df[df['system_ID'].str.contains('NAT')==False]
